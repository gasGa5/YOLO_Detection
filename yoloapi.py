...
def Input_type_yaml(input, sample_num, show = True, save = True):
    source = yaml_data_extract(input,sample_num)
    results = model.predict(source, show = show, save = save)
    return results

def Input_type_URL(input, show = True, save = True):
    results = model.predict(input, show = show, save = save)
    return results

def Input_type_img(input, show = True, save = True):
    results = model.predict(input, show = show, save = save)
    return results
...
results = Input_type_yaml(yaml_file_path,10,save=False)
dection_loop(results)
